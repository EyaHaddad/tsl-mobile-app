...
class RecognitionController {
  ...
  static const int _minFrameDelayMs =
      200; // AUGMENTÉ: 200ms = 5 FPS (Exynos M12 limite)
  ...
  Future<void> start() async {
    ...
    print(
      '📊 [BUFFER] LSTM buffer initialisé (0/${SequenceManager.seqLen} frames)',
    );
    print('═══════════════════════════════════════════════════════════');
    try {
  ...
  Future<void> _processFrame(CameraImage image) async {
    ...
    try {
      ...
      print(
        '📥 [CAMERA] Frame #$_totalFramesReceived | Format: ${image.format.group} | Dims: ${image.width}x${image.height} | Size: ${frameBytes.length} bytes',
      );
      ...
      final result = await processFrameBytes(
        ...
        height: image.height,
        rotation:
            90, // Android camera typically needs 90° rotation for Portrait
      );
      ...
      if (result != null) {
        ...
        print(
          '🎯 [RESULT] ${result.primaryGesture} | Confiance: ${(result.primaryConfidence * 100).toStringAsFixed(1)}%',
        );
  ...
  Future<void> stop() async {
    ...
    print(
      '📊 [BUFFER] Frames restantes au stop: ${sequenceManager.windowLength}/${SequenceManager.seqLen}',
    );
    print('═══════════════════════════════════════════════════════════');
    sequenceManager.reset();
  }
  ...
  Future<RecognitionResultData?> processFrameBytes(
    ...
    try {
      ...
      print(
        '🔄 [PIPE] Appel detectFrameFeatures avec ${encodedFrame.length} bytes (${width}x${height}, rotation: ${rotation}°)...',
      );
      ...
      if (hasValidLandmarks && frameFeatures.isNotEmpty) {
        ...
        if (nonZeroCount < 10) {
          print(
            '⚠️ [SUSPICIOUS_DATA] Only $nonZeroCount non-zero landmarks (expected ~60+)',
          );
        }

        // DÉCLENCHEMENT DE L'INFÉRENCE quand buffer plein ET stride respecté
  ...
  /// ═══════════════════════════════════════════════════════════════════════════
  Future<void> _runInference(List<List<double>> inputSequence) async {
    _isBusy = true;

    try {
      // Sécurité: Vérifier que le buffer est toujours plein
      if (inputSequence.length != SequenceManager.seqLen) {
        print(
          '❌ [LSTM] Invalid buffer size: ${inputSequence.length}/${SequenceManager.seqLen}',
        );
        return;
      }
      ...
      print(
        '🧠 [LSTM] Inférence #$_inferenceCount | Shape: [${inputSequence.length}, ${inputSequence[0].length}]',
      );
      ...
      stopwatch.stop();
      print(
        '✓ [LSTM] Inférence #$_inferenceCount terminée in ${stopwatch.elapsedMilliseconds}ms',
      );
      ...
      if (postProcessedResult != null &&
          postProcessedResult.primaryConfidence > 0.3) {
        print(
          '🎯 [RESULT] ${postProcessedResult.primaryGesture} | Confiance: ${(postProcessedResult.primaryConfidence * 100).toStringAsFixed(1)}%',
        );
  ...
  void debugPrintBufferState() {
    ...
    print(
      '  Frames in buffer: ${sequenceManager.windowLength}/${SequenceManager.seqLen}',
    );
    ...
    if (sequenceManager.windowLength == 0) {
      ...
      print('  Buffer is managed by SequenceManager');
    }
    print('════════════════════════════════════════════════════════════');
  }
  ...
  Future<Uint8List?> _resolveImageBytes(dynamic image) async {
    ...
    if (image is CameraImage) {
      try {
        ...
        print(
          '📸 [DEBUG] Plan Y extrait: ${bytes.length} bytes (${image.width}x${image.height})',
        );
        return bytes;
      } catch (e) {
