...
class InferenceService {
	...
	/// Returns the smoothed prediction or null if window not full
	RecognitionResultData? _applySmoothing(RecognitionResultData raw) {
		...
		// Only apply smoothing when window is full, or fallback to raw
		if (_smoothingWindow.length < config.smoothWindowSize) {
			return raw; // Not enough history yet
		}
		...
		return raw.copyWith(
			...
			debug:
					'${raw.debug} [Smoothed with confidence=$avgConfidence over ${_smoothingWindow.length} frames]',
		);
	}
	...
	InferenceMetrics currentMetrics() {
		...
		final elapsedSeconds = lastTime.difference(firstTime).inMilliseconds / 1000.0;
