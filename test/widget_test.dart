  ...
  group('Tests Intégration : Text-to-Speech & Auto-Delete', () {
    ...
    test('Vérification de la sortie textuelle pour le TTS', () async {
      ...
      expect(
        ...
        isNotNull,
        reason: "Le TTS a besoin d'un texte arabe",
      );
      expect(
        result.primaryGestureAr,
        isNotEmpty,
        reason: "Le texte ne doit pas être vide pour être lu",
      );
    });
    ...
    test('Filtrage des données (Auto-Delete préventif) - Qualité et Période', () {
      ...
      // 2. TEST PÉRIODE (Temps) - Boucle pour les gestes plus vieux que la période
      int joursParametre = 7; // L'utilisateur a choisi 7 jours

      for (int jour = 8; jour <= 14; jour++) {
        int ageDuGeste = jour; // Geste plus vieux que la période de rétention
        ...
        expect(
          ...
          isTrue,
          reason:
              "Geste de $ageDuGeste jours doit être supprimé avec période de $joursParametre jours",
        );
      }
      ...
      expect(
        shouldDiscard,
        isTrue,
        reason: "La donnée doit être jetée si la confiance est trop basse",
      );
    });
